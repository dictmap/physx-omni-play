...
class Trainer:
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        if loadpretrain_denoiser:
            pretrain=load_file('./pretrain/ckpts/ss_flow_img_dit_L_16l8_fp16.safetensors')
            #ipdb.set_trace()
            self.models['denoiser'].load_state_dict(pretrain, strict=False)

            self.models['denoiser'].controlnet.load_state_dict(pretrain, strict=False)
            self.models['denoiser'].controlnet.initialize_weights()

            for name, param in self.models['denoiser'].named_parameters():
                param.requires_grad = False

            for name, param in self.models['denoiser'].controlnet.named_parameters():
    ...
    @torch.no_grad()
    def snapshot(self, suffix=None, num_samples=16, batch_size=1, verbose=False):
        """
