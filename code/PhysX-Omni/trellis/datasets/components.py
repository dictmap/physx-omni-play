...
class ImageConditionedMixin:
    ...
    def get_instance(self, root, instance):
        ...
        if '_1' in instance:
            image_name = instance[:-2]
            image_root=os.path.join('./Partnet-mobility/renders',image_name)
        else:
            image_name=instance[:-1]
            if os.path.exists(os.path.join('./PhysXNet/renders',image_name)):
                image_root=os.path.join('./PhysXNet/renders',image_name)
            if os.path.exists(os.path.join('./Partnet-mobility/renders',image_name)):
                image_root=os.path.join('./Partnet-mobility/renders',image_name)
