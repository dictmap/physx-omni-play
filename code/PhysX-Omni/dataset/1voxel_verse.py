...
if __name__ == "__main__":
    ...
    basepath='./PhysXVerse/'
    os.makedirs('tmp_verse/', exist_ok=True)
    indexlist=os.listdir(os.path.join(basepath,'partobj'))
    logger = get_logger('./tmp_verse/exp_1render'+str(args.ind)+'.log',verbosity=1)
    logger.info('start')
    ...
    for index in indexlist:
        objpath=os.path.join(basepath,'partobj',index)
        tmpdir=os.path.join('tmp_verse/','partobj',index)
        os.makedirs(tmpdir, exist_ok=True)
        os.makedirs(os.path.join('tmp_verse/','finaljson'), exist_ok=True)
        ...
        for name in namelist:
            oripart=load_obj_geometry_fast(os.path.join(objpath,name,name+'.obj'))
        ...
        with open(os.path.join('tmp_verse/','finaljson',index+'.json'),'w') as file:
        ...
        for res in resolist:
            ...
            allind = np.empty((0, 3))  
            for name in namelist:
                oripart1=load_obj_geometry_fast(os.path.join(objpath,name,name+'.obj')) #('model1.obj')
