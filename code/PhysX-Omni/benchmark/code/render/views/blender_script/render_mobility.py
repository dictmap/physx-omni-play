...
def init_render(engine='CYCLES', resolution=512, geo_mode=False):
    ...
    bpy.context.scene.cycles.device = "GPU" if requested_device == "GPU" else "CPU"
    ...
    # Try GPU first by default. Fallback is handled during actual rendering.
    if requested_device == "GPU":
        try:
            ...
            for device in prefs.devices:
                if device.type == 'CUDA':
                    device.use = True
                    print(f"[GPU] Enabled device: {device.name}")
        except Exception as e:
            print(f"[WARN] Failed to enable GPU: {e}")
            bpy.context.scene.cycles.device = "CPU"


def render_with_fallback() -> None:
    """Render current frame; if GPU fails, fallback to CPU and retry once."""
    try:
        bpy.ops.render.render(write_still=True)
        return
    except RuntimeError as exc:
        err = str(exc)
        fallback_markers = ("CUDA kernel", "Unsupported PTX", "OPTIX", "HIP", "METAL")
        if bpy.context.scene.cycles.device == "GPU" and any(marker in err for marker in fallback_markers):
            print("[WARN] GPU render failed, falling back to CPU.")
            bpy.context.scene.cycles.device = "CPU"
            bpy.ops.render.render(write_still=True)
            return
        raise
...
def normalize_scene(args) -> Tuple[float, Vector]:
    ...
    if args.rotate_x:
        scene.rotation_euler[0] += math.radians(args.rotate_x)
...
def main(arg):
    ...
    obj_files = []
    if os.path.isfile(arg.object):
        # 单文件：mesh.glb / texture.glb / sample.glb / merged.obj
        obj_files = [arg.object]
    else:
        # 目录模式，按优先级探测：
        # 1. objs/**/*.obj  — ours_* / articulateanything_* 原始多 part（含子目录）
        obj_files = sorted(glob.glob(os.path.join(arg.object, 'objs', '**', '*.obj'), recursive=True))
        if not obj_files:
            # 2. [0-9]*.glb  — PhysXverse partglb（0.glb, 1.glb, …）
            obj_files = sorted(glob.glob(os.path.join(arg.object, '[0-9]*.glb')))
        if not obj_files:
            # 3. [0-9]*/*.obj — PhysXverse partobj（0/0.obj, 1/1.obj + atlas.png）
            obj_files = sorted(glob.glob(os.path.join(arg.object, '[0-9]*', '*.obj')))
        if not obj_files:
            # 4. PhysX-Mobility/partseg flat objs: objs/*.obj（无子目录）
            obj_files = sorted(glob.glob(os.path.join(arg.object, 'objs', '*.obj')))
    ...
    # normalize scene
    # OBJ-only fixes（GLB/GLTF 由 importer 正确处理，跳过）
    is_obj = any(f.lower().endswith('.obj') for f in obj_files)
    if is_obj:
        # Fix 1: MTL illum=3 被解析为 Metallic=1.0，重置为漫反射
        for mat in bpy.data.materials:
            if mat.use_nodes:
                for node in mat.node_tree.nodes:
                    if node.type == 'BSDF_PRINCIPLED':
                        node.inputs['Metallic'].default_value = 0.0
        print('[INFO] OBJ: Material metallic reset to 0.')

        # Fix 2: 清除 OBJ 文件的显式法线，重新一致性计算
        for obj in bpy.context.scene.objects:
            if obj.type == 'MESH':
        ...
        if meshes:
            ...
            bpy.ops.object.mode_set(mode='OBJECT')
            bpy.ops.object.select_all(action='DESELECT')
        print('[INFO] OBJ: Normals cleared and recalculated.')
    ...
    with open(arg.views_json, "r", encoding="utf-8") as f:
        views = json.load(f)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--views_json', type=str, default='views.json',
                        help='Path to views JSON file (yaw/pitch/radius/fov format). '
                             'Defaults to views.json in CWD.')
    parser.add_argument('--rotate', type=int, default=0, help='Rotate 180 deg around Z axis.')
    parser.add_argument('--rotate_x', type=float, default=0.0, help='Rotate N degrees around X axis (e.g. 90 converts Y-up/Z-fwd to Z-up/Y-back).')
